...
module Statistical
  module Distribution
    ...
    # @attr_reader [Fixnum, Bignum] lower lower bound (inclusive) of the 
    #   distribution
    # @attr_reader [Fixnum, Bignum] upper upper bound (inclusive) of the 
    #   distribution
    # @attr_reader [Fixnum, Bignum] step Step size to use when enumerating 
    ...
    class UniformDiscrete
      ...
      # @return 0 if x doesn't belong to the elements over which the current
      ...
      # @return 0 if k is on the left of the domain, 1 if k on the right and the
      #   value of cdf
      def cdf(k)
        return 0.0 if k < @lower
        return 1.0 if k > @upper
        return ((k - @lower) / @step).floor / count
      end
      ...
      # Returns the expected value of sample variance for the calling instance.
      # 
      # @return Variance of the distribution
      def variance
        n = count
        (n * (2 * n - 1) * @step * @step) / 24.0
      end
      ...
      def eql?(other)
        return false unless other.is_a?(self.class) && @lower == other.lower && @upper == other.upper && @step == other.step
        return true
      end
      
      # @return the count of members over which the distribution exists
      def count
      ...
      #
      # @return Elements over which the distribution exists
      def support
        n = count
